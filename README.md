...
### 已完成的 Bug 修复（v2.48.2，共 1 项致命 bug 修复，涵盖条形码识别模式切换错误）

| # | 严重度 | 描述 | 修复方案 |
|---|--------|------|----------|
| 1 | 🔴 | `ALGORITHM_BARCODE_RECOGNITION` 常量值错误（v2.48.1 改为 254），导致 `switchAlgorithm(254)` 被二哈当作无效值回退到人脸识别模式 | 优先从 `dfrobot_huskylensv2` 库导入算法常量（库中定义了正确值），导入失败时使用手动定义的正确值 11（参考 HuskyLens V2 协议算法编号：1=人脸识别, 2=物体追踪, 3=物体识别, ..., 11=条形码识别） |

### 已完成的 Bug 修复（v2.48.1，共 2 项致命 bug 修复，涵盖搜索药品功能）

| # | 严重度 | 描述 | 修复方案 |
|---|--------|------|----------|
| 1 | 🔴 | `ALGORITHM_BARCODE_RECOGNITION` 常量值错误（原值 2 对应"物体追踪"而非"条形码识别"），导致按搜索药品按钮后二哈切换到物品识别而非条形码识别 | 改为正确的算法编号 254（参考 dfrobot_huskylensv2 官方库算法编号：1=人脸识别, 2=物体追踪, 3=物体识别, 254=条形码识别） |
| 2 | 🔴 | `_enter_search_medicine_impl()` 的 `finally` 块在正常执行完后也会运行，错误地清除了 `_searching_medicine` 状态，导致后续按"返回"按钮时 `exit_search_medicine()` 检查到未 set 而忽略退出调用（日志报"不在搜索药品模式中，忽略退出调用"） | 移除 `finally` 块，改为仅在异常分支中清除状态，正常流程保持搜索状态 |
